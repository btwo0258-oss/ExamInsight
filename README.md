...
✨ 功能模块
模块	功能
👤 用户管理	注册 / 登录 / JWT 认证 / 个人设置（主题切换/模型选择）
📚 知识库管理	创建 / 编辑 / 删除知识库，文档与思维导图统计
📄 文档管理	上传 / 解析 / 分块 / 向量化 / 在线预览 / 下载（PDF/Word/MD/TXT）
...
📎 引用来源展示	展示 AI 回答引用的文档片段（文档名/块序号/相似度/原文）
🧠 思维导图	创建 / 编辑 / 重命名 / 删除，支持关联知识库，节点拖拽/连接线
🛡️ 后台管理	数据统计仪表盘 / 用户封禁解封 / 密码重置 / 忘记密码申请
⚙️ 系统配置	Prompt 模板 / RAG 参数动态调整，无需重启
📁 项目结构
text
...
dashscope:
  api-key: 你的阿里云DashScope_API_Key
💡 API Key 在 阿里云百炼平台 申请
...
cd backend
mvn spring-boot:run
或在 IDEA 中直接运行 LlmApplication.java，启动成功后访问 http://localhost:8080
...
user_settings	用户设置表（主题 / 默认模型）
knowledge_base	知识库表
document	文档表
document_chunk	文档分块表（MySQL 侧备份，主数据在 ES）
conversation	对话表
message	消息表（含版本管理字段 turnId / qVersion / aVersion）
system_config	系统配置表（Prompt 模板 / RAG 参数）
mind_map	思维导图表
