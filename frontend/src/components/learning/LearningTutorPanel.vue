  ...
  <div v-if="open" :class="['learning-tutor', `learning-tutor--${mode}`]">
    ...
    <section class="tutor-surface">
      ...
      <form class="tutor-composer" @submit.prevent="sendQuestion()">
        ...
        <button v-if="messageStore.isStreaming" type="button" aria-label="停止生成" title="停止生成" @click="messageStore.stopStreaming">
          <span class="stop-icon" />
        </button>
        <button v-else type="submit" :disabled="!question.trim() || Boolean(pendingQuestion) || isPreparing" aria-label="发送问题">
...
.stop-icon { width: 10px; height: 10px; border-radius: 1px; background: currentColor; }
