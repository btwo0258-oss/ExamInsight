...
async function submit() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : '创建知识库失败'
  } finally {
    creating.value = false
  }
}
...
.library-create-modal .create-error { margin: -4px 0 14px; color: var(--color-danger); font-size: 13px; }
