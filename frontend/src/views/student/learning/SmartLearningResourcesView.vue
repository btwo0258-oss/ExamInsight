...
const generationLabels: Record<string, string> = {
  QUEUED: '等待生成',
  PREPARING_SOURCE: '正在整理资料',
  GENERATING_CONTENT: 'AI 正在生成内容',
  REPAIRING_FORMAT: '正在修复格式（1/1）',
  VALIDATING: '正在校验格式',
  FINALIZING: '正在保存结果',
  READY: '已就绪',
  FAILED: '生成失败',
...
function generationLabel(resource: { status: string; generationStage: string }) {
  if (resource.status === 'READY') return '已就绪'
  if (resource.status === 'FAILED') return '生成失败'
  return generationLabels[resource.generationStage] || '正在准备'
}
...
async function retryActiveResource() {
  const resource = activeResource.value
  if (!resource || resource.status !== 'FAILED' || retryingResource.value) return
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : '资源重试失败，请稍后再试。'
  } finally { retryingResource.value = false }
}
...
async function load() {
  ...
  try {
    ...
    const job = store.current?.activeJob
    if (job?.kind === 'RESOURCE_PREPARATION' && ['QUEUED', 'RUNNING'].includes(job.status)) {
      void store.waitForJob(job.jobId, projectId).catch((error) => {
        if (store.activeProjectId === projectId) {
          errorMessage.value = error instanceof Error ? error.message : '资源准备失败，请重试。'
        }
      })
    }
  } catch (error) {
...
<template>
  <StudentShell>
    <main class="resources-page">
      ...
      <div v-if="activeResource?.status === 'FAILED'" class="resource-retry-banner"><span>{{ activeResource.errorMessage || '这份资源生成失败。' }}</span><button type="button" :disabled="retryingResource" @click="retryActiveResource">{{ retryingResource ? '正在重试…' : '重试这份资源' }}</button></div>
...
.resource-retry-banner{width:min(1280px,100%);display:flex;align-items:center;justify-content:space-between;gap:12px;margin:14px auto 0;padding:10px 12px;border:1px solid color-mix(in srgb,var(--color-danger) 30%,var(--color-border));border-radius:10px;color:var(--color-danger);background:var(--color-surface)}.resource-retry-banner button{min-height:34px;padding:0 12px;border:1px solid var(--color-border);border-radius:8px;color:inherit;background:var(--color-surface);cursor:pointer}.resource-retry-banner button:disabled{opacity:.55;cursor:default}
.item-progress,.detail-progress{display:block;overflow:hidden;border-radius:999px;background:var(--ui-hover-strong-bg)}.item-progress{width:100%;height:3px;margin-top:2px}.detail-progress{width:min(280px,70%);height:5px;margin-top:4px}.item-progress b,.detail-progress b{display:block;height:100%;border-radius:inherit;background:var(--color-text);transition:width .35s ease}
</style>
