...
const groupResult = ref<Awaited<ReturnType<typeof learningStore.submitExerciseGroup>>>()
...
async function submitQuizGroup() {
  if (operationPending.value) return
  operationPending.value = true
  actionError.value = ''
  try {
    groupResult.value = await learningStore.submitExerciseGroup(plan.value.id, taskExercises.value.map((item) => item.id))
  } catch (error) {
...
async function createFollowupTask() {
  if (!activeTask.value || !followupMode.value || operationPending.value) return
  operationPending.value = true
  actionError.value = ''
  try {
    const result = await learningStore.createAdaptivePracticeTask(plan.value.id, activeTask.value.id, {
    ...
    actionError.value = error instanceof Error ? error.message : '追加练习生成失败'
  } finally {
    operationPending.value = false
  }
}
...
<template>
  <StudentShell v-if="isLoading || loadError || !hasPlan">
    <LearningRouteState
      :loading="isLoading"
      :error="loadError"
      :has-plan="hasPlan"
