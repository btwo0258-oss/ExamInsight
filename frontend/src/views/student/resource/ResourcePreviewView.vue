...
const resourceId = computed(() => typeof route.params.resourceId === 'string' ? route.params.resourceId : '')
...
const statusMessage = computed(() => {
  ...
  if (preview.value?.status === 'processing') return preview.value.errorMessage || '文件仍在处理中，请稍后重试'
  if (preview.value?.status === 'too_large') return preview.value.errorMessage || '文件较大，暂不支持在线预览'
  if (preview.value?.status === 'unsupported') return preview.value.errorMessage || '当前格式暂不支持在线预览'
  if (preview.value?.status === 'failed') return preview.value.errorMessage || '文件预览加载失败'
  return ''
})
...
function findLearningResource(id: string, externalKey?: string): { plan: LearningPlan; resource: LearningResource } | undefined {
...
function applyLearningPreview(value: ResourcePreviewDto) {
  ...
  if (learningResource.content) value.textContent = learningResource.content
    ...
    value.textContent = [
      `# ${plan.title}学习方案`, '',
      '## 学习目标', plan.goal, '',
      '## 个性化画像', plan.profile.map((item) => `- ${item.label}：${item.value}`).join('\n'), '',
      '## 学习路径',
...
async function download() {
  ...
  try {
    ...
    anchor.href = url
    anchor.download = resource.value.name
    anchor.click()
  ...
  <StudentShell>
    <section class="resource-preview-workspace">
      <header class="preview-header">
        ...
        <button class="download-button" type="button" :disabled="!resource || downloading" @click="download">
      ...
      <main class="preview-stage">
        ...
        <section v-else-if="!canRender" class="preview-state">
          ...
          <h1>{{ preview?.status === 'too_large' ? '文件较大，暂不支持在线预览' : '无法在线预览' }}</h1>
          <p>{{ statusMessage }}</p>
          <button v-if="resource" type="button" @click="download"><AppIcon name="download" :size="17" />下载文件</button>
        </section>

        <div v-else-if="preview?.previewKind === 'presentation' && presentation" class="presentation-pages">
          <article v-for="page in presentation.previewPages" :key="page.id" class="presentation-page">
            <span>第 {{ page.order }} 页</span>
            <PresentationSlidePreview :page="page" :aspect-ratio="presentation.config.aspectRatio" />
          </article>
        </div>
        ...
        <section v-else-if="preview?.previewKind === 'spreadsheet'" class="spreadsheet-document">
          ...
          <div v-if="activeSheet" class="sheet-table-wrap">
            <table>
              ...
              <tbody><tr v-for="(row, rowIndex) in activeSheet.rows" :key="rowIndex"><td class="row-number">{{ rowIndex + 1 }}</td><td v-for="(_, index) in activeSheet.columns" :key="index">{{ row[index] ?? '' }}</td></tr></tbody>
            </table>
          </div>
        ...
        <section v-else-if="preview?.previewKind === 'audio' && preview.previewUrl" class="audio-document">
          ...
          <article v-if="preview.transcript"><h2>识别文本</h2><p>{{ preview.transcript }}</p></article>
        </section>
      </main>
    </section>
  </StudentShell>
...
.resource-preview-workspace * { box-sizing: border-box; }
.resource-preview-workspace { height: 100%; display: flex; flex-direction: column; overflow: hidden; background: var(--color-hover); color: var(--color-text); }
.preview-header { flex: 0 0 60px; padding: 0 20px; display: grid; grid-template-columns: 38px minmax(0, 1fr) auto; align-items: center; gap: 12px; border-bottom: 1px solid var(--color-border); background: var(--color-surface); }
.header-icon { width: 36px; height: 36px; display: grid; place-items: center; border: 0; border-radius: 50%; background: transparent; color: var(--color-text-muted); cursor: pointer; }
.header-icon:hover { background: var(--color-hover); color: var(--color-text); }
.preview-breadcrumb { min-width: 0; display: flex; align-items: center; gap: 8px; font-size: 13px; }
.preview-breadcrumb span { color: var(--color-text-muted); }
.preview-breadcrumb strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.download-button,
.preview-state button { min-height: 36px; padding: 0 13px; display: inline-flex; align-items: center; justify-content: center; gap: 7px; border: 1px solid var(--color-border); border-radius: 6px; background: var(--color-surface); color: var(--color-text); cursor: pointer; font: inherit; font-size: 13px; font-weight: 700; }
.download-button:hover:not(:disabled),
.preview-state button:hover { background: var(--color-hover); }
.download-button:disabled { cursor: not-allowed; opacity: .5; }
.preview-stage { flex: 1; min-height: 0; overflow: auto; padding: 28px clamp(18px, 4vw, 64px) 56px; }
.preview-state { width: min(520px, 100%); min-height: 320px; margin: 8vh auto 0; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; }
.preview-state h1,
.preview-state p { margin: 0; }
.preview-state h1 { margin-top: 18px; font-size: 20px; }
.preview-state p { max-width: 460px; margin-top: 8px; color: var(--color-text-muted); font-size: 13px; line-height: 1.7; }
.preview-state button { margin-top: 20px; }
.state-icon { width: 58px; height: 58px; display: grid; place-items: center; border-radius: 50%; background: var(--color-surface); box-shadow: var(--shadow-sm); }
.paper-document,
.spreadsheet-document,
.mindmap-document,
.audio-document { width: min(1040px, 100%); margin: 0 auto; border: 1px solid var(--color-border); border-radius: 8px; background: var(--color-surface); box-shadow: var(--shadow-sm); }
.paper-document { min-height: 76vh; padding: clamp(32px, 6vw, 88px); overflow-wrap: anywhere; }
.text-document { margin: 0 auto; white-space: pre-wrap; color: var(--color-text); font: 14px/1.75 ui-monospace, SFMono-Regular, Consolas, monospace; }
.image-document { display: block; max-width: min(1200px, 100%); max-height: calc(100vh - 150px); margin: 0 auto; object-fit: contain; border-radius: 6px; box-shadow: var(--shadow-sm); }
...
.sheet-tabs button.active { border-bottom-color: var(--color-text); color: var(--color-text); font-weight: 700; }
.sheet-table-wrap { overflow: auto; max-height: calc(100vh - 185px); }
.sheet-table-wrap table { width: 100%; border-collapse: collapse; font-size: 13px; }
.sheet-table-wrap th,
.sheet-table-wrap td { min-width: 120px; padding: 9px 10px; border: 1px solid var(--color-border); text-align: left; white-space: nowrap; }
.sheet-table-wrap th { position: sticky; top: 0; z-index: 1; background: var(--color-hover); }
.sheet-table-wrap .row-number { min-width: 50px; width: 50px; color: var(--color-text-muted); text-align: center; }
.mindmap-document { height: calc(100vh - 150px); min-height: 560px; overflow: hidden; }
.audio-document { min-height: 380px; padding: 48px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
.audio-document h1 { margin: 18px 0 26px; font-size: 20px; }
.audio-document audio { width: min(560px, 100%); }
.audio-document article { width: min(720px, 100%); margin-top: 30px; padding-top: 24px; border-top: 1px solid var(--color-border); }
.audio-document article h2 { font-size: 15px; }
.audio-document article p { color: var(--color-text-muted); line-height: 1.75; }
.empty-preview { min-height: 300px; display: grid; place-items: center; color: var(--color-text-muted); }
.spin { animation: spin .8s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

@media (max-width: 720px) {
  .preview-header { padding: 0 10px; grid-template-columns: 38px minmax(0, 1fr) 38px; }
  .download-button { width: 36px; padding: 0; font-size: 0; }
  .preview-stage { padding: 16px 10px 32px; }
  .paper-document { min-height: 80vh; padding: 28px 20px; }
  .pdf-document,
  .mindmap-document { height: calc(100vh - 110px); min-height: 480px; }
}
</style>
