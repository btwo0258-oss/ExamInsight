...
const setAnswer = ref('')
const setResult = ref<ReturnType<typeof learningStore.submitWrongReviewSet>>()
const knowledgeFilter = ref('全部知识点')
const statusFilter = ref<'全部' | '需巩固' | '已掌握'>('全部')
const searchText = ref('')
const retakeMode = ref(false)
const retakeAnswer = ref('')
const retakeResult = ref<ReturnType<typeof learningStore.reviewWrongQuestion>>()
...
function submitRetake() {
  if (!wrong.value || !retakeAnswer.value) return
  retakeResult.value = learningStore.reviewWrongQuestion(plan.value.id, wrong.value.id, retakeAnswer.value)
  retakeMode.value = false
}

function createReinforcement() {
  if (!wrong.value) return
  const set = learningStore.createWrongReviewSet(plan.value.id, [wrong.value.id], {
...
function submitSet() {
  if (!activeSet.value) return
  setResult.value = learningStore.submitWrongReviewSet(plan.value.id, activeSet.value.id)
}
        ...
        <section class="mistake-layout">
          <aside class="wrong-index">
            ...
            <div class="index-filters">
              ...
              <select v-model="knowledgeFilter" aria-label="知识点筛选"><option v-for="item in knowledgeOptions" :key="item">{{ item }}</option></select>
          ...
          <article v-if="wrong && exercise" class="wrong-detail">
            ...
            <template v-if="!retakeMode">
              ...
              <section v-if="reinforcementOpen" class="reinforcement-config">
                ...
                <label><span>题量</span><select v-model="reinforcementCount"><option :value="2">2 题</option><option :value="3">3 题</option><option :value="5">5 题</option></select></label>
                <label><span>难度</span><select v-model="reinforcementDifficulty"><option>保持难度</option><option>逐步提升</option></select></label>
                <button class="primary-btn" type="button" @click="createReinforcement">生成并开始</button>
              </section>
            </template>
          </article>
...
<style scoped>
.mistakes-page{min-height:100%;padding:28px 34px 48px;background:var(--color-bg);box-sizing:border-box}.mistakes-page *{box-sizing:border-box}h1,h2,h3,p{margin:0}button,input,select{font:inherit}.page-head,.mistakes-content,.empty-state{max-width:1480px;margin:0 auto}.page-head{display:grid;grid-template-columns:42px 1fr auto;align-items:center;gap:16px}.back-button{width:42px;height:42px;border:1px solid var(--color-border);border-radius:10px;background:var(--color-surface);cursor:pointer}.page-head h1{color:var(--color-text);font-size:30px}.page-head p{margin-top:6px;color:var(--color-text-muted)}.primary-btn,.outline-btn{min-height:40px;border-radius:9px;padding:0 18px;font-weight:750;cursor:pointer}.primary-btn{border:1px solid var(--color-primary);background:var(--color-primary);color:#fff}.outline-btn{border:1px solid var(--color-border);background:var(--color-surface);color:var(--color-text)}button:disabled{opacity:.45;cursor:not-allowed}.mistakes-content{margin-top:22px}.mistake-toolbar{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:12px}.filter-group{display:flex;gap:8px;flex-wrap:wrap}.filter-group select,.search-box{height:38px;border:1px solid var(--color-border);border-radius:9px;background:var(--color-surface);color:var(--color-text)}.filter-group select{padding:0 30px 0 10px}.search-box{display:flex;align-items:center;gap:7px;padding:0 10px}.search-box input{width:210px;border:0;outline:0;background:transparent;color:var(--color-text)}.mistake-toolbar>p{color:var(--color-text-muted);font-size:13px}.mistake-layout{display:grid;grid-template-columns:340px minmax(0,1fr);gap:14px;align-items:start}.wrong-index,.wrong-detail,.filtered-empty,.empty-state{border:1px solid var(--color-border);border-radius:12px;background:var(--color-surface);box-shadow:var(--shadow-sm)}.wrong-index{max-height:calc(100vh - 190px);overflow:auto;padding:10px}.wrong-index>header{position:sticky;top:-10px;z-index:2;display:flex;justify-content:space-between;padding:10px 8px 12px;background:var(--color-surface);color:var(--color-text)}.wrong-index>header span{color:var(--color-text-muted)}.wrong-item{width:100%;display:grid;grid-template-columns:8px 1fr;gap:9px;padding:12px 9px;border:0;border-top:1px solid var(--color-border);background:transparent;text-align:left;cursor:pointer}.wrong-item.active{border-radius:9px;background:#f1f5f9}.wrong-item>div{display:grid;gap:5px;min-width:0}.wrong-item strong{overflow:hidden;color:var(--color-text);font-size:14px;text-overflow:ellipsis;white-space:nowrap}.wrong-item small{color:var(--color-text-muted);font-size:12px}.wrong-item footer{grid-column:2;display:flex;justify-content:space-between;gap:8px}.wrong-item footer>span{color:#475569;font-size:12px;font-weight:700}.status-dot{width:7px;height:7px;margin-top:5px;border-radius:50%;background:#f59e0b}.status-dot.status-已掌握{background:#22c55e}.status-dot.status-复习中{background:#3b82f6}.status-dot.status-反复出错{background:#ef4444}.wrong-detail{min-height:680px;padding:26px 30px}.detail-head{display:flex;align-items:center;justify-content:space-between}.detail-head>div{display:flex;gap:8px}.detail-head span,.knowledge-section span{border-radius:999px;padding:5px 10px;background:#f1f5f9;color:#475569;font-size:12px}.status-label{font-size:13px;color:#d97706}.status-label.status-已掌握{color:#16a34a}.status-label.status-反复出错{color:#dc2626}.question-section{margin-top:24px}.question-section>small,.analysis-section>small,.knowledge-section>small,.history-section>small{color:var(--color-text-muted);font-weight:700}.question-section h2{max-width:920px;margin-top:8px;color:var(--color-text);font-size:22px;line-height:1.55}.question-section pre{margin:16px 0;padding:16px;border-radius:9px;background:#f8fafc;overflow:auto}.option-list{display:grid;gap:9px;margin-top:18px}.option-list label{display:flex;align-items:center;gap:10px;min-height:46px;padding:10px 13px;border:1px solid var(--color-border);border-radius:9px;color:var(--color-text)}.option-list label.selected{border-color:var(--color-primary);background:#eff6ff}.option-list label.wrong{border-color:#fecaca;background:#fef2f2;color:#b91c1c}.option-list label.right{border-color:#bbf7d0;background:#f0fdf4;color:#15803d}.answer-comparison{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:20px}.answer-comparison section{display:grid;gap:7px;padding:14px;border-radius:9px}.answer-wrong{border:1px solid #fecaca;background:#fef2f2;color:#b91c1c}.answer-right{border:1px solid #bbf7d0;background:#f0fdf4;color:#15803d}.analysis-section,.knowledge-section,.history-section{margin-top:18px;padding-top:18px;border-top:1px solid var(--color-border)}.analysis-section h3{margin-top:9px;color:var(--color-text);font-size:18px}.analysis-section p{margin-top:8px;color:#475569;line-height:1.75}.knowledge-section div{display:flex;gap:8px;margin-top:10px}.history-section>div{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--color-border);color:var(--color-text-muted)}.history-section strong{color:#dc2626}.history-section strong.correct{color:#16a34a}.detail-actions,.retake-actions{display:flex;gap:10px;margin-top:22px}.retake-actions{justify-content:flex-end}.retake-feedback{margin-top:18px;padding:13px;border:1px solid #fecaca;border-radius:9px;background:#fef2f2;color:#b91c1c}.retake-feedback.correct{border-color:#bbf7d0;background:#f0fdf4;color:#15803d}.retake-feedback p{margin-top:5px}.reinforcement-config{display:flex;align-items:end;gap:10px;flex-wrap:wrap;margin-top:14px;padding:15px;border-radius:9px;background:#f8fafc}.reinforcement-config>div{display:grid;gap:4px;min-width:280px;margin-right:auto;color:var(--color-text)}.reinforcement-config small{color:var(--color-text-muted)}.reinforcement-config label{display:grid;gap:4px;color:var(--color-text-muted);font-size:12px}.reinforcement-config select{height:38px;border:1px solid var(--color-border);border-radius:8px;background:#fff;padding:0 9px}.filtered-empty,.empty-state{display:grid;justify-items:center;text-align:center;padding:70px 24px}.filtered-empty p,.empty-state p{max-width:620px;margin:10px 0 20px;color:var(--color-text-muted);line-height:1.7}.empty-state{margin-top:28px;min-height:520px;align-content:center}.empty-state>span{display:grid;place-items:center;width:66px;height:66px;margin-bottom:16px;border-radius:50%;background:#ecfdf3;color:#16a34a}.empty-state h2,.filtered-empty h2{color:var(--color-text)}
.page-head,.module-tabs,.mistakes-content,.sets-content,.empty-state{max-width:1480px;margin-left:auto;margin-right:auto}.page-head{grid-template-columns:42px 1fr}.module-tabs{display:flex;gap:24px;margin-top:20px;border-bottom:1px solid var(--color-border)}.module-tabs button{display:flex;align-items:center;gap:7px;padding:0 2px 12px;border:0;border-bottom:2px solid transparent;background:transparent;color:var(--color-text-muted);font-weight:750;cursor:pointer}.module-tabs button.active{border-bottom-color:var(--color-primary);color:var(--color-text)}.module-tabs span{display:grid;place-items:center;min-width:21px;height:21px;border-radius:999px;background:#eef2f7;font-size:12px}.mistakes-content,.sets-content{margin-top:14px}.index-filters{display:grid;gap:8px;padding:0 2px 12px}.index-filters .search-box{width:100%;background:var(--color-bg)}.index-filters .search-box input{width:100%}.index-filters>select{width:100%;height:38px;padding:0 10px;border:1px solid var(--color-border);border-radius:9px;background:var(--color-bg);color:var(--color-text)}.status-filters{display:flex;flex-wrap:wrap;gap:6px}.status-filters button{min-height:28px;padding:0 9px;border:1px solid var(--color-border);border-radius:999px;background:transparent;color:var(--color-text-muted);font-size:11px;cursor:pointer}.status-filters button.active{border-color:#cbd5e1;background:#eef2f7;color:var(--color-text);font-weight:750}.wrong-index{max-height:calc(100vh - 180px)}.wrong-item footer>span{display:flex;align-items:center;gap:6px}.wrong-item footer em,.detail-status em{border-radius:999px;padding:2px 6px;background:#fef2f2;color:#dc2626;font-size:10px;font-style:normal;font-weight:750}.detail-status{display:flex;align-items:center;gap:8px}.status-dot.status-已掌握{background:#22c55e}.status-label.status-已掌握{color:#16a34a}.filtered-detail-empty{display:grid;align-content:center;justify-items:center;min-height:680px;text-align:center}.filtered-detail-empty h2{color:var(--color-text);font-size:20px}.filtered-detail-empty p{margin-top:10px;color:var(--color-text-muted)}.sets-content .wrong-detail{min-height:620px}.status-dot.mastered{background:#22c55e}.set-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px}.set-head small,.set-head p{color:var(--color-text-muted)}.set-head h2{margin:5px 0;color:var(--color-text)}.set-head>strong{color:#d97706}.set-start,.set-result{display:grid;justify-items:center;align-content:center;min-height:450px;text-align:center}.set-start>span,.set-result>strong{font-size:46px;font-weight:800;color:var(--color-text)}.set-start p,.set-result p{max-width:540px;margin:10px 0 20px;color:var(--color-text-muted)}.set-answer-grid{display:flex;flex-wrap:wrap;gap:7px;margin:22px 0 12px}.set-answer-grid button{width:34px;height:34px;border:1px solid var(--color-border);border-radius:8px;background:#fff;color:var(--color-text);cursor:pointer}.set-answer-grid button.answered{background:#ecfdf3;color:#15803d}.set-answer-grid button.active{border-color:var(--color-primary);box-shadow:0 0 0 2px #dbeafe}.set-detail .question-card{margin-top:8px}.set-empty{margin-top:14px}@media(max-width:1000px){.mistake-layout{grid-template-columns:1fr}.wrong-index{max-height:320px}.page-head{grid-template-columns:42px 1fr}.answer-comparison{grid-template-columns:1fr}}@media(max-width:640px){.mistakes-page{padding:20px 14px}.wrong-detail{padding:20px 16px}.search-box{width:100%}.search-box input{width:100%}.page-head{align-items:start}}
.retake-card-wrap{display:grid;gap:10px;margin-top:20px}.retake-card-wrap>.outline-btn{justify-self:end}.answer-comparison pre{max-height:240px;margin:0;padding:10px;border-radius:7px;background:#fff;overflow:auto;white-space:pre-wrap}.answer-comparison code{font-family:"Cascadia Code",Consolas,monospace;font-size:12px}
</style>
