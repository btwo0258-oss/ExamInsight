...
const isLearningChat = computed(
  () => route.query.learning === "1" || learningProjectId.value !== null,
);
const isTutorChat = computed(
  () =>
    learningProjectId.value !== null &&
    (route.query.tutor === "1" ||
...
const tutorQuestionFromRoute = computed(() =>
  typeof route.query.tutorQuestion === "string" ? route.query.tutorQuestion.trim() : "",
);
const learningProject = computed(() =>
  learningProjectId.value ? learningStore.getPlan(learningProjectId.value) : null,
);
...
const availableKnowledgeBases = computed(() => {
  return knowledgeBaseStore.list.map((item) => ({
    id: item.id,
    name: item.name,
...
const selectedKnowledgeBase = computed(() =>
  availableKnowledgeBases.value.find((item) => item.id === selectedKnowledgeBaseId.value),
);
...
const failedTurnId = computed(() => {
  const failed = [...messages.value]
    .reverse()
    .find((message) => message.role === "assistant" && message.errorMsg);
  return failed?.turnId || null;
});
const canRetry = computed(() =>
  Boolean(retryAction.value || (activeChatId.value && failedTurnId.value)),
);
...
const confirmationReady = computed(() => {
  ...
  const documentMessage = messages.value.find(
    (message) => message.id === currentDocumentMessageId.value,
  );
  return Boolean(
    documentMessage?.kind === "learning-document" &&
    !documentMessage.learningData?.loading &&
    documentMessage.learningData?.content,
  );
});
...
function appendLearningMessage(
...
function buildTutorContext() {
  ...
  if (!project)
    return "你是当前学习项目的 AI 助教。请围绕用户问题给出准确、清晰、可执行的学习指导。";
  const scheme = project.resources.find((resource) => resource.group === "学习方案");
  const stages = project.stages
    .map((stage) => {
      const tasks = stage.tasks
        .map((task) => `- ${task.title}（${task.status ?? (task.done ? "已完成" : "未开始")}）`)
        .join("\n");
      return `### ${stage.title}\n${stage.desc}\n${tasks}`;
    })
    .join("\n\n");
  const wrongPoints =
    [...new Set(project.wrongQuestions.flatMap((wrong) => wrong.knowledge))].join("、") ||
    "暂无错题";
...
async function ensureTutorConversation() {
  ...
  let matched =
    current?.id === storedId ||
    (current?.learningProjectId === project.id && current?.title === title)
      ? current
      : (conversationStore.list.find((item) => item.id === storedId) ??
        conversationStore.list.find(
          (item) => item.learningProjectId === project.id && item.title === title,
        ));
  if (isMockDataSource && !matched && Number.isFinite(storedId) && storedId > 0) {
    conversationStore.restoreLearningConversation(
      storedId,
      project.id,
      project.title,
  ...
  conversationStore.linkLearningProject(
    conversationId,
    project.id,
    project.title,
    "learning-tutor",
...
async function sendTutorQuestion(question: string, files?: File[]) {
  ...
  conversationStore.linkLearningProject(
    conversationId,
    learningProject.value.id,
    learningProject.value.title,
    "learning-tutor",
  );
}
...
async function ensureLearningConversation(text: string) {
  ...
  await router.replace({
    path: `/chat/${conversationId}`,
    query: { learningProjectId: String(projectId) },
  });
  return conversationId;
}
  ...
  try {
    ...
    if (isLearningSetupChat.value) {
      ...
      learningProfile.value = (
        await requestLearningProfile(`${learningPrompt.value} ${text}`)
      ).profile;
      if (learningPhase.value === "document") {
        appendLearningMessage(
          conversationId,
          "assistant",
          "好的，我已根据你的补充更新学习画像和方案确认稿。",
        );
...
async function confirmLearningProfile(messageId: string) {
  ...
  learningTimer = window.setTimeout(async () => {
    try {
      ...
      reportFlowError(error, "学习方案确认稿生成失败", () =>
        regenerateLearningDocument(documentMessage.id),
      );
    }
  }, 1050);
}
...
function selectKnowledgeBase(id: number | null) {
  ...
  learningProfile.value.source =
    availableKnowledgeBases.value.find((item) => item.id === id)?.name || "无";
  ...
  learningTimer = window.setTimeout(async () => {
    try {
      ...
      const generated = await learningStore.createPlan({
        ...
        studyDepth: profile.preferences.includes("项目实操")
          ? "项目实操"
          : profile.preferences.includes("练习驱动")
            ? "刷题强化"
            : "系统学习",
        questionCount: 60,
      ...
      if (activeChatId.value !== conversationId) return;
      learningPhase.value = "document";
      reportFlowError(error, "学习方案生成失败", () =>
...
watch(
  ...
  (id) => {
    ...
    conversationStore.setCurrentId(chatId);
  },
  { immediate: true },
);
...
watch(
  ...
  (items) => {
    ...
    const lastDocument = [...items]
      .reverse()
      .find((message) => message.kind === "learning-document");
  ...
  <StudentShell>
    <section class="student-chat">
      ...
      <div class="student-chat__body">
        ...
        <div
          v-if="isLearningSetupChat && confirmationReady && !decisionDismissed"
          class="learning-decision-wrap"
        >
        ...
        >
          <template v-if="isLearningSetupChat && showWelcome" #context>
            <div class="learning-context">
              <button
                class="learning-context__trigger ui-hover-row"
                type="button"
              ...
              <div v-if="knowledgeMenuOpen" class="knowledge-menu ui-menu-panel">
                ...
                >
                  ...
                  ><span>无</span
                  ><AppIcon v-if="selectedKnowledgeBaseId === null" name="check" :size="15" />
                </button>
                <button
                  ...
                  class="ui-menu-item"
                  type="button"
                  :aria-selected="selectedKnowledgeBaseId === item.id"
                  @click="selectKnowledgeBase(item.id)"
                >
                  <span class="ui-menu-icon"><AppIcon :name="item.icon" :size="16" /></span
                  ><span>{{ item.name }}</span
                  ><AppIcon v-if="selectedKnowledgeBaseId === item.id" name="check" :size="15" />
                </button>
                <div class="ui-menu-divider" />
                <button
                  class="knowledge-menu__create ui-menu-item"
                  type="button"
                  @click="
                    knowledgeMenuOpen = false;
                    knowledgeMenuQuery = '';
                    knowledgeCreateOpen = true;
                  "
                >
...
.learning-context__trigger:hover {
  background: var(--color-hover);
  color: var(--color-text);
}
.learning-context__trigger span {
  overflow: hidden;
...
.knowledge-menu > button .ui-menu-icon {
  flex: 0 0 var(--ui-menu-icon-size);
}
.knowledge-menu > .knowledge-menu__create {
  color: var(--color-primary);
}

.learning-decision-wrap {
...
.learning-decision strong,
.learning-decision span {
  display: block;
}
.learning-decision strong {
  margin-bottom: 2px;
...
.learning-decision__actions {
  display: flex;
  gap: 7px;
}
.learning-decision__actions button {
  height: 34px;
  padding: 0 12px;
  border: 1px solid var(--color-border);
  border-radius: 8px;
...
.learning-decision__actions button.primary {
