  ...
  while (true) {
    ...
    while (true) {
      ...
      for (const evt of events) {
        ...
        // 后端使用 SseEmitter.event().data(delta) 发送纯文本，不指定 event 名称
        // 此时 evt.event 为 undefined，应正常处理
        if (evt.event && evt.event !== "message" && evt.event !== "delta") continue;
