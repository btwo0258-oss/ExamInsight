...
import type { LibraryResourceDto, LibraryResourceProcessingStatus } from '@/types/contracts/library'
...
export interface LibraryResourceRepository {
  ...
  upload(file: File, libraryId: number | null, projectId?: number | null): Promise<LibraryResourceDto>
...
const mockRepository: LibraryResourceRepository = {
  ...
  async list(libraryId) {
    ...
    return libraryId === undefined ? resources : resources.filter((item) => item.libraryId === libraryId)
  },
  saveMock(resources) {
  ...
  async remove(id) {
    mockSession.set(DOMAIN, mockSession.get(DOMAIN, initialMockResources()).filter((item) => item.id !== id))
  },
  async retry(id) {
    const resources = mockSession.get(DOMAIN, initialMockResources())
...
const apiRepository: LibraryResourceRepository = {
  ...
  async list(libraryId) {
    ...
    return ((response.data?.data ?? response.data) as Record<string, unknown>[]).map(normalizeResource)
  },
  saveMock() {},
  async upload(file, libraryId, projectId = null) {
  ...
  async download(id) {
    const response = await request.get(`/api/library/resources/${id}/download`, { responseType: 'blob' })
