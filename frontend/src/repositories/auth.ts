...
function loadMockUsers(): MockUser[] {
  const fallback: MockUser[] = [{ id: 1, username: 'admin', password: '123456', nickname: 'Admin', avatar: null }]
  try {
...
function normalizeAuth(payload: unknown): AuthResult {
  ...
  return {
    ...
    user: {
      ...
      nickname: nestedUser.nickname === undefined || nestedUser.nickname === null ? null : String(nestedUser.nickname),
      avatar: nestedUser.avatar === undefined || nestedUser.avatar === null ? null : String(nestedUser.avatar),
    },
  }
}
...
const mockAuthRepository: AuthRepository = {
  async login(payload) {
    ...
    return { token: `mock_${Date.now()}_${Math.random().toString(16).slice(2)}`, user: publicUser(user) }
  },
  async register(payload) {
    ...
    const user: MockUser = { id: Date.now(), username, password: payload.password, nickname, avatar: null }
    users.unshift(user)
    saveMockUsers(users)
    return { token: `mock_${Date.now()}_${Math.random().toString(16).slice(2)}`, user: publicUser(user) }
  },
...
const apiAuthRepository: AuthRepository = {
  ...
  async forgotPassword(username) {
    return (await request.post('/api/user/forgot-password', { username })).data as { message: string }
  },
  async getSettings() {
