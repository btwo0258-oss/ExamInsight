...
-- 数据库迁移脚本 v2
-- 执行时间：2026-07-16
-- 说明：添加后端实体需要的字段和智能学习模块表
-- ============================================

-- 1. 修改 conversation 表，添加缺失字段
ALTER TABLE `conversation`
ADD COLUMN `is_pinned` tinyint NOT NULL DEFAULT 0 COMMENT '是否置顶：0否 1是' AFTER `total_tokens`,
ADD COLUMN `learning_project_id` bigint NULL DEFAULT NULL COMMENT '关联的学习项目ID' AFTER `is_pinned`,
ADD COLUMN `conversation_type` varchar(50) NULL DEFAULT 'general' COMMENT '对话类型：general-普通对话, learning-setup-学习配置, learning-tutor-学习助教' AFTER `learning_project_id`;

-- 添加索引
ALTER TABLE `conversation`
ADD INDEX `idx_learning_project_id`(`learning_project_id`),
